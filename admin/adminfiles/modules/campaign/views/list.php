  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Campaign List</h2>
    </header>
         ...
         <tr class="gradeX">
           ...
           <td class="actions">
            ...
            // if($this->session->userdata("usergroup") == 1 || $this->session->userdata("clms_company") == $acc->company_id)
              echo anchor('campaign/edit/'.$acc->id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('campaign/delete/'.$acc->id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));?>
          </td>
        </tr>
        <?php
      } ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>campaign/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>campaign/cascadeAction",
          type:"post",
          data:jsonData,
