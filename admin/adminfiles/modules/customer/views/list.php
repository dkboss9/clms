  ...
  <table class="table table-bordered table-striped mb-none" id="datatable-default">
    ...
    <tbody>
      ...
      foreach ($status->result() as $row) {
        ...
        ?>
        <tr class="gradeX">
         <td><input type="checkbox" class="checkone" value="<?php echo $row->customer_id; ?>" /></td>
         ...
         <td class="actions">
          ...
          <?php echo anchor('customer/edit/'.$row->customer_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('customer/delete/'.$row->customer_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));?>
        </td>
      </tr>
      <?php
    } ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>customer/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>customer/cascadeAction",
          type:"post",
          data:jsonData,
