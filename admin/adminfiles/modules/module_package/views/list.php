  ...
  <table class="table table-bordered table-striped mb-none" id="datatable-default">
    ...
    <tbody>
      ...
      foreach ($status->result() as $row) {
        ...
        ?>
        <tr class="gradeX">
         <td><input type="checkbox" class="checkone" value="<?php echo $row->package_id; ?>" /></td>
         <td><?php echo $row->name;?></td>
         <td><?php echo $row->max_no_company;?></td>
         <td class="actions">
          <?php echo anchor('module_package/edit/'.$row->package_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish;?>
          &nbsp; <a href="<?php echo base_url("module_package/permission/".$row->package_id); ?>"><span class="glyphicon glyphicon-asterisk" aria-hidden="true"></span></a> &nbsp;
        </td>
      </tr>
      <?php
    } ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('.fa-support').tooltip({'placement': 'bottom','title':'Assign Dashboard Sections'});
    $(".glyphicon-asterisk").tooltip({'placement': 'bottom','title':'Package permission'});
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>module_package/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>module_package/cascadeAction",
          type:"post",
          data:jsonData,
