...
// Load the download helper and send the file to your desktop
	...
	function calendar(){
		...
		if($timeid==0)
			...
			$time = $timeid;
		$events = $this->dashboardmodel->getMyEvents($time,$this->session->userdata("clms_userid"));				
	...
	for($i=0; $i< $total_rows; $i++){
						...
						if($this->session->userdata("clms_userid")==''){
							...
							if($this->input->post('action') && $this->input->post('action')=='submit'){
								$mydata = array('event_title'=>$this->input->post('title'),
									...
									'modified_by'=>$this->session->userdata("clms_userid"),
		...
		//Get User Details
					...
					function stripe_payment($amount,$company_id){
						...
						redirect('admin/profile/'.$company_id,'location');
					}
