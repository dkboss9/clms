  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Task Status List</h2>
    </header>
        ...
        foreach ($status->result() as $row) {
          ...
          <tr class="gradeX">
           ...
           <td class="actions">
            ...
            <?php ;
            echo $publish.'&nbsp;';
            if($row->company_id == $this->session->userdata("clms_company")){
              echo anchor('task_status/edit/'.$row->status_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;';
              echo anchor('task_status/delete/'.$row->status_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));
            }
            ?>
          </td>
        </tr>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>task_status/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>task_status/cascadeAction",
          type:"post",
          data:jsonData,
