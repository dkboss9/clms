...
class Email_template extends CI_Controller{
	function __construct(){
		...
		$this->module_code = 'EMAIL-TEMPLATE';
	}
	function index(){
	...
	function listall(){
		if($this->session->userdata("clms_userid")!='' && $this->usermodel->checkperm($this->module_code,"VIEW")){
			$config['base_url'] 		= base_url().'index.php/email_template/listall';
			...
			$query = $this->emailmodel->listall();
			$config['total_rows'] = $query->num_rows();
			$this->pagination->initialize($config);
			$query->free_result();
			$page = $this->uri->segment(3,0);			
			$limit = array("start"=>$config['per_page'],"end"=>$page);	
			$query = $this->emailmodel->listall($limit);
			if($query->num_rows()>0){
				$i = 1;
		...
		if($this->session->userdata("clms_userid")!='' && $this->usermodel->checkperm($this->module_code,"ADD")){
			if($this->input->post('submit')){
				...
				if($this->form_validation->run()==FALSE){
					...
					$details['news']['modified_by'] 	= $this->session->userdata("clms_userid");
					$this->emailmodel->insert($details['news']);
		...
		if($this->session->userdata("clms_userid")!="" && $this->usermodel->checkperm($this->module_code,"EDIT")){
			if($this->input->post('submit')){
				...
				if($this->form_validation->run()==FALSE){
					...
					$this->emailmodel->update($email_id,$details['news']);
				...
				$query = $this->emailmodel->loaddata($id);
				if($query->num_rows()==1){
					$row = $query->row();
	...
	function changestatus(){
		if($this->session->userdata("clms_userid")==''){
			...
			$this->emailmodel->update($id,$value['details']);
			redirect('news/listall/','location');
		}
	}
	function cascadeAction() {
		...
		$ids = $data['ids'];
		$action = $data['action'];
		$query = $this->emailmodel->cascadeAction($ids, $action);
	}	
}
?>
