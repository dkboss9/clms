...
class Quotemodel extends CI_Model{
	...
	function listall($status='',$expired_date='',$studentid=null){	
		...
		if($this->session->userdata("clms_company") && $this->session->userdata("clms_company") != "")	
			$this->db->where("q.company_id",$this->session->userdata("clms_company"));	

		if($all_data == 0){
			$this->db->where("(q.added_by = $userid OR q.customer_id = $company_user_id)");
		}

		if($studentid){
			$this->db->where("q.customer_id",$studentid);
		}
