...
class Quote extends CI_Controller {
  ...
  {
    $num = $this->customermodel->check_duplicateEmail($str,$this->session->userdata("clms_company"));
    if ($num > 0)
    {
...
if($this->input->post("customer_id")){
  $customer = $this->input->post("customer_id");
  redirect("project/quote/$customer");
}elseif($this->input->post("is_project") > 0){
...
function mail_preview($quoteid){
  ...
  $quote = $this->db->where("quote_id",$quoteid)->get("quote")->row();
  $this->session->set_flashdata('success_message', 'Quote Sent successfully');
  if($this->input->post("project") > 0)
  redirect('project/quote/'.$quote->customer_id);
  else
  redirect('quote/listall');
