    ...
    <section class="panel">
      ...
      <div class="panel-body">
        <form class="form-horizontal form-bordered" id="form" action="<?php echo base_url("quote/add");?>" method="post" enctype='multipart/form-data'>
         <div class="form-group" <?php echo $this->input->get("customer") ? 'style="display:none;"' : ''; ?>>
           ...
           ?>
             <select name="customer" id="customer" data-plugin-selectTwo class="form-control required" >
               ...
               foreach ($customers->result() as $row) {
                 ?>
                 <option value="<?php echo $row->id;?>" <?php echo $row->id == @$student->company_student_id ? 'selected' : '';?>><?php echo $row->first_name;?> <?php echo $row->last_name;?></option>
                 <?php
               }
               ?>
             </select>
<?php }else{ ?>
  <input type="text" name="name"  class="form-control" value="<?php echo $student->first_name;?> <?php echo $student->last_name;?>" readonly >
  <input type="hidden" name="customer" value="<?php echo @$student->company_student_id; ?>" >
<?php } ?>
           </div>
...
<div class="col-md-1">
      <a href="javascript:void(0);" id="link_quotefrom"><i class="glyphicon glyphicon-plus"></i></a>
   </div>
</div>
...
<div class="form-group">
  ...
  <div class="col-md-6">
    ...
    <input type="hidden" name="customer_id" value="<?php echo $this->input->get("customer")??0;?>">
    <input type="submit" name="submit" value="submit" class="mb-xs mt-xs mr-xs btn btn-success">
    <?php if($this->input->get("project")) { ?>
              <a href="<?php echo base_url("project/quote/".$this->input->get("customer"));?>" class="mb-xs mt-xs mr-xs btn btn-danger">Back</a>
              <?php }else{ ?>
                <a href="<?php echo base_url("quote");?>" class="mb-xs mt-xs mr-xs btn btn-danger">Back</a>
             <?php } ?>
  </div>
</div>
</form>
...
<script type="text/javascript">
 ...
 $(document).ready(function(){
  ...
  $("#link_quotefrom").click(function(){
   $('#form_quotefrom_model').modal();
 })
