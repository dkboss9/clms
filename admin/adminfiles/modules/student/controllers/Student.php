...
class Student extends CI_Controller {
  ...
  function listall() {
    ...
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
      $companyid = $this->session->userdata("clms_company");
      $data['student'] 	= $this->studentmodel->listall();
      ...
      $data['campaigns'] = $this->studentmodel->get_campaigns($companyid);
      $data['goal_types'] = $this->studentmodel->goal_types($companyid);
      $data['users'] = $this->studentmodel->get_users_new($companyid);
      $this->load->helper("functions");
      $data['campaign_status'] = campaign_status();
      $data['campaign_priority'] = campaign_priority();
      $data['total_contacts'] = $this->studentmodel->count_contact($companyid);

      $uuid = $this->input->get("studentid")??null;

      $data["detail"] = $this->studentmodel->get_studentdetail_with_uuid($uuid)->row();

      $data['page'] 			= 'student_detail';
      $this->load->vars($data);
  ...
  function add_campaign(){
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"ADD")) {
      ...
      $data['values']['campaign_name']	= $this->input->post('name');
      $data['values']['start_date']  = date("Y-m-d", strtotime($this->input->post('start_date')));
      $data['values']['company_id']      = $this->session->userdata("clms_company");
      $data['values']['status'] 		= $this->input->post("status");
      $data['values']['end_date'] 		= date("Y-m-d", strtotime($this->input->post('end_date')));

      $data['values']['detail'] 		= $this->input->post("detail");
      $data['values']['priority'] 		= $this->input->post("priority");

      $this->db->insert("lead_campaign", $data['values']);

      $id = $this->db->insert_id();

      if($this->input->post("txt_number")){
          $txt_numbers = $this->input->post("txt_number");
          $goal_types = $this->input->post("goal_type");
          $goal_names = $this->input->post("goal_name");
          $end_dates = $this->input->post("goal_end_date");

          foreach($txt_numbers as $key => $number){
              $goal_data = $this->db->order_by("id","desc")->get("pnp_lead_campaign_goals")->row();
              $this->db->insert("pnp_lead_campaign_goals",[
                  "lead_campaign_id" => $id,
                  "goal_number" => $number,
                  "goal_type" => $goal_types[$key],
                  "name" => $goal_names[$key],
                  "start_date" => !empty($goal_data) ?  $goal_data->start_date : date("Y-m-d", strtotime($this->input->post('start_date'))),
                  "end_date" => date("Y-m-d",strtotime($end_dates[$key]))
              ]);
          }
      }
      ...
      if($this->input->post("assign_to")){
          ...
          $assign_tos = $this->input->post("assign_to");

          foreach($assign_tos as $key => $assign_to){
              $this->db->insert("pnp_lead_campaign_leadby",[
                  "lead_campaign_id" => $id,
                  "user_id" => $assign_to
              ]);
          }
      ...
      if($this->input->post("txt_files")){

          $attachments = $this->input->post("txt_files");

          foreach($attachments as $key => $attachment){
              $this->db->insert("lead_campaign_attachments",[
                  "lead_campaign_id" => $id,
      ...
      $this->session->set_flashdata('success_message', ' campaign added successfully');
      redirect('student/listall');
  ...
  function student_detail(){
    ...
    $data['documents'] = $this->studentmodel->getDoccuments($id);
    $data['qualifications'] = $this->studentmodel->getQualifications($id);
    $data['experiences'] = $this->studentmodel->getExperinces($id);
    echo $this->load->view("edit_client",$data,true);
...
function add_contact(){
  if($this->session->userdata("clms_userid")!="" && $this->usermodel->checkperm($this->module_code,"ADD")){
    ...
    $value['details']['is_client']         = 'No';

    $value['details']['campaign_id']         = $this->input->post("campaign");
    ...
    $this->db->set("company_id",$this->session->userdata("clms_company"));
    $this->db->update("company_students");

    $insert_array = array(
      "company_student_id" => $student,
      "account_setting_country" => $this->input->post("country"),
   ...
   if($this->form_validation->run()!=FALSE){
    ...
    //  print_r( $type[0]); die();
       foreach($_FILES as $key => $value) {
        ...
        if(!empty($_FILES[$key]['name']) && !in_array($key, ["profile_pic"])){ 
    ...
    $qualifaction = $this->input->post("qualification");
...
function resend_activation($id){
  $this->studentmodel->sendemail($id);
  $request = [
      "company_user_id" => $id,
      "type" => 'Student',
      "added_by" => $this->session->userdata("clms_userid"),
...
function delete() {
  if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"DELETE")) {
    ...
    $cond = array("id"=>$delid);
    $content = $this->usermodel->getDeletedData('pnp_company_students',$cond);
    $logs = array(
