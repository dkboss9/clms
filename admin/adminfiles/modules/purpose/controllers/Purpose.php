...
class Purpose extends MX_Controller {
    ...
    function listall() {
        if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
            $data['access'] 	= $this->purposemodel->listall();
            $data['page'] 			= 'list';
            $this->load->vars($data);
    ...
    function add() {
        if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"ADD")) {
            ...
            $this->purposemodel->add($data['values']);
            $logs = array(
            ...
            $this->session->set_flashdata('success_message', 'purpose added successfully');
...
function addpurpose(){
    ...
    $userdata = $this->session->userdata("clms_userid");
    $date = date("Y-m-d");
    $data['values']['type_name']    = $purpose_name;
    $data['values']['company_id']      = $this->session->userdata("clms_company");
    $data['values']['added_date']       = time();
    $data['values']['added_by']         = $userdata;
    $data['values']['modified_date']    = time();
    ...
    $purpose = array("purpose_id"=>$id,"purpose_name"=>$purpose_name);
...
function edit() {
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"EDIT")) {
        if ($this->input->post('submit')) {
            ...
            $data['values']['modified_date']    = time();
            $data['values']['modified_by']      = $userdata;
            $this->purposemodel->update($id, $data['values']);
            $logs = array(
                "content" => serialize($data['values']),
                "action" => "Edit",
                "module" => "Manage Lead Type",
            ...
            $query = $this->purposemodel->getdata($id);
            if ($query->num_rows() > 0) {
...
function delete() {
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"DELETE")) {
        ...
        $this->usermodel->insertUserlog($logs);
        $this->purposemodel->delete($delid);
        $this->session->set_flashdata('success_message', 'purpose deleted successfully');
        redirect('purpose/listall');
    } else {
        $this->session->set_flashdata('error', 'Please login with your username and password');
        redirect('login', 'location');
...
function cascadeAction() {
    ...
    $query = $this->purposemodel->cascadeAction($ids, $action);
