<?php
class Visa_class extends MX_Controller {

    function __construct() {
        parent::__construct();
        $this->container = 'main';
        $this->load->model('visa_classmodel');
        $this->load->model('users/usermodel');
        $this->module_code = 'visa_class';
    }

    function index() {
        if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
            redirect('visa_class/listall', 'location');
        } else {
    ...
    function listall() {
        if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
            $data['access'] 	= $this->visa_classmodel->listall();
            $data['page'] 			= 'list';
            $this->load->vars($data);
    ...
    function addVisaClass(){
        $name = $this->input->post("visa_class_name");
        ...
        $this->visa_classmodel->add($data['values']);
    ...
    function add() {
        if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"ADD")) {
           ...
           /*  if(!$this->session->userdata("clms_company") || $this->session->userdata("clms_company") == ""){
            ...
            $this->session->set_flashdata('success_message', 'Visa Subclass added successfully');
            redirect('visa_class/listall');
        }else{
            $data['page'] = 'add';
...
function edit() {
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"EDIT")) {
        if ($this->input->post('submit')) {
            ...
            $data['values']['modified_date']    = time();
            $data['values']['modified_by']      = $userdata;
            $this->visa_classmodel->update($id, $data['values']);
            $logs = array(
                "content" => serialize($data['values']),
                "action" => "Edit",
                "module" => "Manage Lead Type",
            ...
            $this->usermodel->insertUserlog($logs);
            $this->session->set_flashdata('success_message', 'Visa Subclass edited Successfully');
            redirect('visa_class/listall');
        } else {
            $id = $this->uri->segment(3);
            $query = $this->visa_classmodel->getdata($id);
            if ($query->num_rows() > 0) {
...
function delete() {
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"DELETE")) {
        ...
        $content = $this->usermodel->getDeletedData('visa_class',$cond);
        $logs = array(
        ...
        $this->usermodel->insertUserlog($logs);
        $this->visa_classmodel->delete($delid);
        $this->session->set_flashdata('success_message', 'Visa Subclass deleted successfully');
        redirect('visa_class/listall');
    } else {
        $this->session->set_flashdata('error', 'Please login with your username and password');
        redirect('login', 'location');
...
function cascadeAction() {
    ...
    $query = $this->visa_classmodel->cascadeAction($ids, $action);
