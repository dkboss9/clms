...
class Company extends MX_Controller {
  ...
  function listall() {
     if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
      ...
      // $data['appointments'] = $this->appointmentmodel->listall("","","","","","","","","","");
            //print_r($this->session->userdata); die();
      if($this->session->userdata("usergroup")=='1' || $this->session->userdata("usergroup")=='2'){ 
        $data['page'] 			= 'list';
      }
      else{
...
function save_tabname(){
  if ($this->session->userdata("clms_userid") != "") {
    $company_id = $this->session->userdata("clms_userid");
...
function add(){
  if($this->session->userdata("clms_userid")!="" && $this->usermodel->checkperm($this->module_code,"ADD")){
   ...
   if($this->form_validation->run()!=FALSE){
    ...
    $value['details']['verified_at']         = date("Y-m-d H:i:s");
    ...
    if($this->input->post("is_admin")){
     ...
     $this->db->set("company_id",$this->session->userdata("clms_userid"));
   }
...
function opening_hours($id){
  if($this->session->userdata("clms_userid")!=""){
    if($this->input->post("submit")){
      ...
      $this->db->set("time_interval",$this->input->post("time_slot"));
      $this->db->update("company_details");
...
function check_slug($userid){
  $this->db->where("userid !=",$userid);
  $this->db->where("user_slug",$this->input->post("slug"));
  echo $this->db->get("pnp_users")->num_rows();
}
...
function invoice_setting($company_id){
  if(!$this->session->userdata("clms_userid") ||  $this->session->userdata("clms_userid") == '')
    redirect('login', 'location');
  if($this->input->post("submit")){
      $this->db->where('company_id',$company_id);
