  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Head agents </h2>
    </header>
         ...
         <tr class="gradeX">
          ...
          <td class="actions">
            ...
            if($acc->company_id > 0 OR $this->session->userdata("usergroup") == '1')
              echo anchor('head_agents/edit/'.$acc->id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('head_agents/delete/'.$acc->id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));
     
            $number = $this->employeemodel->count_requestSent($acc->id,"Head Agent");
            if(!$acc->clms_front_userid){
            ?>
            <a href="<?php echo base_url("head_agents/resend_activation/".$acc->id)?>" class="link_email"><i class="fa fa-envelope" aria-hidden="true"></i>(<?php echo $number->num_rows();?>)</a>
          <?php } ?>
          </td>
        </tr>
        <?php
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>head_agents/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>head_agents/cascadeAction",
          type:"post",
          data:jsonData,
