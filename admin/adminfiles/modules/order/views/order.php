   ...
   <?php echo $this->load->view("tab");?>
                              ...
                              <section class="panel">
                                ...
                                <div class="div_menu">
                                  ...
                                  foreach ($status as $row){
                                   $order_no = $this->ordermodel->countOrderwithStatus($row->threatre_id);
                                   ?> 
                                   <a href="<?php echo base_url("dashboard/order?status=".$row->threatre_id);?>"  class="mb-xs mt-xs mr-xs btn btn-default"><?php echo $row->name;?>(<?php echo $order_no;?>)</a>
                                   <?php
                                 }
                                 ?>
                                 ...
                                 foreach ($invoice_status as $row){
                                  $order_no = $this->ordermodel->countOrderwithInvoiceStatus($row->status_id);
                                  ?> 
                                  <a href="<?php echo base_url("dashboard/order?invoice=".$row->status_id);?>"  class="mb-xs mt-xs mr-xs btn btn-default"><?php echo $row->status_name;?>(<?php echo $order_no;?>)</a>
                                  <?php
                                }
                                $archived_num = $this->ordermodel->countArchiveOrder();
                                ?>
                                <a href="<?php echo base_url("dashboard/order?archived=1");?>"  class="mb-xs mt-xs mr-xs btn btn-default">Archived (<?php echo $archived_num;?>)</a>
                                 <a href="<?php echo base_url("dashboard/order/report");?>"  class="mb-xs mt-xs mr-xs btn btn-primary">Sale's Report</a>
                              </div>
                            </div>
                          </section>
                          <div class="col-sm-12">
                        ...
                        <table class="table table-bordered table-striped mb-none" id="datatable-default">
                          ...
                          <tbody>
                            ...
                            <td>
                             <span class="label" style="color:#fff;background:<?php echo @$invoice->color_code;?>"><?php echo @$invoice->status_name;?></span>
                           </td>
                           <td>
                             <?php 
                             ...
                             if(($row->order_status == 17 || $row->order_status == 18 || $row->order_status == 21) && ($row->invoice_status == 8)){
                              ?>
                              <a href="javascript:void(0);" onclick='return  confirmation("Confirmation","Are you sure you want to archive the order? ","Cancel","Confirm","<?php echo base_url("order/make_archive/".$row->order_id);?>");' class="btn btn-primary " >Archive</a>
                              <?php
                          ...
                          <td>
                            ...
                            if($row->price == 0){
                              ?>
                              <a href="javascript:void(0);" class="link_update_price btn btn-primary" rel="<?php echo $row->order_id;?>" title=""  > Update Price </a>
                              <?php } ?>
                            </td>

                            <td>
                              <a href="<?php echo base_url("order/customer_note/".$row->order_id);?>" class="link_update btn btn-primary" rel="<?php echo $row->order_id;?>" title="" > Note </a>
                            </td>

                            <td class="actions">
                              <?php echo anchor('order/duplicate/'.$row->order_id.'?tab=1',' <i class="fa fa-copy"></i>',array("class"=>"")).'&nbsp;'.anchor('order/edit/'.$row->order_id.'?tab=1','<span class="glyphicon glyphicon-edit"></span>',array("class"=>"")).'&nbsp;'.$publish; //.'&nbsp;'.anchor('order/delete/'.$row->order_id.'?tab=1','<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));?>

                              <?php if(count($install) > 0){?>
                              <a class="  change_install" href="#" onclick='return  confirmation_popup("Confirmation","Do you really want to change it? ","Cancel","Confirm","<?php echo base_url("order/install/".$row->order_id);?>");'> 
                                <span class="glyphicon glyphicon-user" <?php echo count($install) > 0 ? 'style="color:red;"':''; ?>></span>
                              </a>
                              <?php 
                            }else{
                              ?>
                              <a class="simple-ajax-popup-reminder  " href="<?php echo base_url("order/install/".$row->order_id);?>"> 
                                <span class="glyphicon glyphicon-user" <?php echo count($install) > 0 ? 'style="color:red;"':''; ?>></span>
                              </a>
                              <?php
                            } ?>

                            <a class="simple-ajax-popup123 " href="<?php echo base_url("order/payment/".$row->order_id);?>"><span class="fa fa-dollar"></span></a>
                            <a href="<?php echo base_url("order/download_order/".$row->order_id);?>" class=""><i class="glyphicon glyphicon-download-alt"></i></a>
                            <a href="<?php echo base_url("order/details/".$row->order_id);?>?tab=1" class=""><span class="glyphicon glyphicon-fullscreen"></span></a>
                            ...
                            <a href="<?php echo base_url("order/mail_preview/".$row->order_id);?>?tab=1" class="link_email simple-ajax-popup123 "><span class="glyphicon glyphicon-envelope"> </span> (<?php echo  $counter;?>)</a>
