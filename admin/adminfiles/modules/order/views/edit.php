  ...
  <div class="col-lg-12">
    <section class="panel">
      <header class="panel-heading">
        ...
        <h2 class="panel-title">order : [Edit] - Order No: <?php echo $quote->order_number;?></h2>
      </header>
      <div class="panel-body">
        <form class="form-horizontal form-bordered" id="form" action="<?php echo base_url("order/edit");?>" method="post" enctype='multipart/form-data'>
          <?php if(isset($error)){ ?>
          <div class="alert alert-danger">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
             ...
             <div class="col-md-3">
               <select name="customer" id="customer" data-plugin-selectTwo data-plugin-selectTwo class="form-control">
                 ...
                 foreach ($customers->result() as $row) {
                   ?>
                   <option value="<?php echo $row->id;?>" <?php if(@$quote->customer_id == $row->id) echo 'selected="selected"';?>><?php echo $row->first_name.' '.$row->last_name;?></option>
                   <?php
                 }
                 ?>
               </select>
      ...
      <a href="javascript:void(0);" id="add_more" class="btn btn-primary list-btn">Add more package</a>
    </div>
...
<div class="form-group">
 <label class="col-md-2 control-label" for="price">Due</label>
 <div class="col-md-3">
...
<div class="form-group">
 ...
 <div class="col-md-3">
   ...
   <label class="col-md-2 control-label div_payment_term" for="price" <?php if(@$quote->finance_option != '1') echo 'style="display:none;"'; ?>>Payment Terms</label>
   <div class="col-md-3 div_payment_term" <?php if(@$quote->finance_option != '1') echo 'style="display:none;"'; ?>>
 ...
 <div class="col-md-3">
   <select name="order_status" id="order_status" data-plugin-selectTwo class="form-control required" >
    ...
    foreach ($status as $row){
      ?>
      <option value="<?php echo $row->threatre_id;?>" <?php if(@$quote->order_status == $row->threatre_id) echo 'selected="selected"';?>><?php echo $row->name;?></option>
      <?php
    }
    ?>
  </select>
...
<div class="col-md-3">
  <select name="invoice_status" id="invoice_status" data-plugin-selectTwo class="form-control required" >
    ...
    foreach ($invoices as $row){
      ?>
      <option value="<?php echo $row->status_id;?>" <?php if(@$quote->invoice_status == $row->status_id) echo 'selected="selected"';?>><?php echo $row->status_name;?></option>
      <?php
    }
    ?>
...
<div class="form-group">
  ...
  <div class="col-md-6">
    ...
    <a href="<?php echo base_url("project/invoice/".$quote->customer_id);?>" class="mb-xs mt-xs mr-xs btn btn-danger">Back</a>
              <?php }else{ ?>
