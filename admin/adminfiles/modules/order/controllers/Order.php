...
class order extends MX_Controller {
  ...
  function index() {
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
      redirect('order/listall', 'location');
    } else {
  ...
  function order_details($id){
    ...
    if ($this->session->userdata("clms_userid") != "" &&  $this->usermodel->checkperm($this->module_code,"VIEW")) {
      ...
      if($this->input->get("to_date"))
        ...
        $to_date= '';
      //echo $this->input->get("type"); die();
      if($this->input->get("type") == "inhouse"){
      ...
      $data['search_referral'] = $referral;
      $data['search_status'] = $ostatus;
      $data['from_date'] = $from_date;
      $data['to_date'] = $to_date;
      $data['archived'] = $ostatus;
      ...
      $data['invoice'] = $invoice;
      $data['order'] 	= $this->ordermodel->listall($ostatus,$invoice,$archived,$order_date,$referral,$from_date,$to_date,'','',$type);
    // echo $this->db->last_query(); die();
      // $data['inhouse_order'] 	= $this->ordermodel->listall($ostatus,$invoice,$archived,$order_date,$referral,$from_date,$to_date,'','','inhouse');
      $data['status'] = $this->ordermodel->listOrderStatus();
      $data['invoice_status'] = $this->ordermodel->listInvoices();
      $data['company'] = $this->quotemodel->getCompanyDetails($this->session->userdata("clms_company"));
      $this->load->model("referals/referal_model");
      $data['referral_users'] = $this->referal_model->listall();
      if($this->session->userdata("usergroup")  == 9)
        $data['page'] 			= 'order_referral';
      else
        $data['page'] 			= 'list';
      $data['nav'] = 'job';
      $this->load->vars($data);
      ...
      if($this->input->post("submit")){
        ...
        if( $this->input->post("paid") != ""){
          $insert_paid = array(
            ...
            "payment_method" => $this->input->post("payment_method"),
            "note" => $this->input->post("note"),
            "receipted_by" => $this->session->userdata("clms_userid"),
          );
          $this->db->insert("invoice_payment",$insert_paid);
       ...
       $order = $this->ordermodel->getdata($this->input->post("invoice_id"))->row();
       if($this->input->post("project") == '1'){
         $this->session->set_flashdata('success_message', 'Payment done successfully');
         redirect("project/invoice/".$order->customer_id,'location');
       }else{
         $this->session->set_flashdata('success_message', 'Payment done successfully');
         redirect($_SERVER["HTTP_REFERER"],'location');
       }
  ...
  function notes(){
    if($this->input->post("submit")){
      if($this->input->post("note") != ''){
        ...
        $order =$this->ordermodel->listallInstall_detail($this->input->post("order_id"));
        $this->db->insert("order_installer_notes",$note);
        if($this->session->userdata("usergroup") == 11){
        $notification = array(
          "content" => $this->session->userdata("username").' has added note in the  Order. #'.$order->order_number,
          "link" => 'dashboard/installer/',
          "from_id" => $this->session->userdata("clms_userid"),
          "to_id" => $order->added_by,
          "added_date" => date("Y-m-d")
        );
        $this->db->insert("company_notification",$notification);
      }else{
        $assignees = $this->ordermodel->getorder_installer($this->input->post("order_id"));
        foreach($assignees as $assignee){
          $install_user = $this->ordermodel->get_install_user($assignee->installer_id);
          //echo $this->db->last_query();
          $notification = array(
            "content" => $this->session->userdata("username").' has added note in the  Orderr. #'.$order->order_number,
            "link" => 'dashboard/installer/',
            "from_id" => $this->session->userdata("clms_userid"),
            "to_id" => $install_user->user_id,
            "added_date" => date("Y-m-d")
          );
          $this->db->insert("company_notification",$notification);
        }
     ...
     $data['installers'] = $this->ordermodel->getorder_installer($id);
   //  echo $this->db->last_query();
     $data['order_id'] = $id;
...
function install(){
  if($this->input->post("submit")){
    ...
    $this->db->where("order_id",$order_id);
    $this->db->delete("pnp_order_assignees");
    $assignees = $this->input->post("installer");
    if(!empty($assignees)){
      foreach($assignees as $assignee){
        $install_arr = [
          "order_id" => $order_id,
          "installer_id" => $assignee,
          "order_installer_id" =>  $install_id
        ];
        $this->db->insert("pnp_order_assignees",$install_arr);
        $install_user = $this->ordermodel->get_install_user($assignee);
        //echo $this->db->last_query();
        $notification = array(
          "content" => $this->session->userdata("username").' has allocated the  Order. #'.$order->order_number,
          "link" => 'dashboard/installer/',
          "from_id" => $this->session->userdata("clms_userid"),
          "to_id" => $install_user->user_id,
          "added_date" => date("Y-m-d")
    ...
    if($this->input->post("project") > 0){
      redirect("project/invoice/".$order->customer_id,"");
    }else{
      redirect("order/installer","");
    }
   ...
   // echo $this->db->last_query(); 
    ...
    $data['result'] = $this->ordermodel->getdata($id)->row();
    $data["customer"] = $this->quotemodel->getCustomer($data['result']->customer_id);
...
function installer(){
  if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm("installer-order","VIEW")) {
    $this->load->model("installer_status/installer_statusmodel");
    $data['installer_status'] = $this->installer_statusmodel->listall();
    $data['company'] = $this->quotemodel->getCompanyDetails($this->session->userdata("clms_company"));
    $data['install'] 	= $this->ordermodel->listallInstall();
    $data['installer'] 	= $this->ordermodel->listallInstall();
    $data['installer_activity'] 	= $this->ordermodel->listallInstall(1);
    //  echo $this->db->last_query(); die();
    $data['assigned_installer'] 	= $this->ordermodel->listallInstall();
    $data['page'] 			= 'installer';
    $data['nav'] = 'installer';
    $this->load->vars($data);
    $this->load->view($this->container);
...
function callendar_install(){
  if($this->input->post("submit")){
    ...
    // $deadline = explode('/',$this->input->post("deadline_date"));
    $order_installer = $this->ordermodel->getOrderInstallers($order_id);
    //print_r($order_installer); die();
    $install_id = $order_installer->id;
    $installer = array(
      "order_id"=>$order_installer->order_id,
    //  "installer"=> $this->input->post("installer"),
      "install_type"=> $order_installer->install_type,
      "installed_date" => $order_installer->installed_date,
      "deadline_date" => $order_installer->deadline_date,
      "installed_time" => $order_installer->installed_time,
      "deadline_time" => $order_installer->deadline_time,
      "time_allocate_by" => $order_installer->time_allocate_by,
      "hourly_rate" => $order_installer->hourly_rate,
      "total_hour" => $order_installer->total_hour,
      "flat_amount" => $order_installer->flat_amount,
      "fuel_amount" => $order_installer->fuel_amount,
      "transport_amount" => $order_installer->transport_amount,
      "others_amount" => $order_installer->others_amount,
      "total_amount" => $order_installer->total_amount,
      "payment_method" => $order_installer->payment_method,
      "allocated_time" => $order_installer->allocated_time,
      "assign_by" =>$order_installer->assign_by,
      "install_status" =>$order_installer->install_status,
    );
    ...
    $assignees = $this->ordermodel->getorder_installer($order_id);
    $assignees = array_column($assignees,"installer_id");
    //print_r( $assignees);die();
...
function activity_report(){
  if ($this->session->userdata("clms_userid") != "" ) {
    ...
    $data["installers"] = $this->ordermodel->getInstallers();
    $data["installer_type"] = $this->ordermodel->getInstallerType();
    // $data['invoices'] = $this->supportmodel->get_invoiceList();
    $data['company'] = $this->quotemodel->getCompanyDetails($this->session->userdata("clms_company"));
    $data['install'] 	= $this->ordermodel->listallInstall();
    // $data['referrals'] = $this->referral_request_model->listall();
    $data["installers"] = $this->ordermodel->getInstallers();
    ...
    if($this->input->get("custom") && $this->input->get("custom") != "")
    ...
    if($this->input->get("order_id") && $this->input->get("order_id") != "")
      $orderid = $this->input->get("order_id");
    else
      $orderid = '';

    if($this->input->get("contractor") && $this->input->get("contractor") != "")
      $contractor = $this->input->get("contractor");
    else
      $contractor = '';
    ...
    $data['activities'] = $this->ordermodel->listActivities($custom,$date,$from_date,$to_date,$orderid,$contractor);
    //echo $this->db->last_query(); die();
    $data['nav'] = 'installer';
    $data['page'] 			= 'activity_report';
    $this->load->vars($data);
    $this->load->view($this->container);
  } else {
    $this->session->set_flashdata('error', 'Please login with your username and password');
...
$activities = $this->ordermodel->listActivities($custom,$date,$from_date,$to_date,$orderid,$contractor);
...
$header = array("Date","Order Number","Nature of project","Customer","Description","Date & Time","Deadline","Start time","End time","Contractor"); 
fputcsv($file, $header);
foreach ($activities->result() as $key=>$row){ 
  $customer = $this->quotemodel->getCustomer($row->customer_id);
  $data = [
    ...
    @$customer->customer_name,
    $row->description,
    date("d/m/Y",strtotime($row->installed_date)),
...
function edit_activity($id){
  ...
  if($this->input->post("submit")){
    $this->db->where("id",$id);
    $this->db->update("pnp_order_installer_activities",[
      "start_time" => $this->input->post("start_time"),
      "end_time" => $this->input->post("end_time"),
      "description" => $this->input->post("note")
    ]);

    $this->session->set_flashdata('success_message', 'Daily Acitvity has been updated successfully');
   
    redirect("dashboard/activity_report","");
  }else{
    $activity = $this->ordermodel->get_activitydetail($id);
    if($this->session->userdata("clms_userid") != $activity->user_id){
      $this->session->set_flashdata('error_message', 'You don\'t have permission to edit this activity.');
      redirect("dashboard/activity_report","");
    }
    $orderid = $activity->order_id;
    $data["order_installer"] = $this->ordermodel->getOrderInstallers($orderid);
    $data["activity"]  = $activity;
    $data['order'] = $this->ordermodel->getdata($orderid)->row();
    $data["customer"] = $this->customermodel->getdata($data['order']->customer_id)->row();
    $data['page'] = 'edit_activity'; 
    $data['heading'] = 'Edit Activities';
...
function get_order_detail(){
  ...
  $customer = $this->quotemodel->getCustomer($order['customer_id'])->row();
...
if($this->input->post("customer_id")){
  ...
  redirect("project/invoice/$customer");
}else{
  redirect('order/listall');
}
...
if($this->input->get("is_existing") == 2){
  $data['page'] = 'add_order_inhouse';
}else{
...
function admin_note($order_id){
 if($this->input->post("submit")){
  ...
  if($admin != ""){
    $admin_array = array("order_id"=>$order_id,"admin_note"=>$admin,"added_date"=>time(),"added_by"=>$this->session->userdata("clms_userid"));
...
function customer_note($order_id){
  if($this->input->post("submit")){
    ...
    if($customer != ""){
      ...
      $customer_array = array("order_id"=>$order_id,"customer_note"=>$customer,"added_date"=>time(),"added_by"=>$this->session->userdata("clms_userid"));
...
$customer = $this->input->post("customer");
 ...
 if ($query->num_rows() > 0) {
  ...
  $data['cust'] = $this->quotemodel->getCustomer($data['quote']->customer_id);
  $data["quote_inverters"] = $this->ordermodel->getInverterOrder($id);
  if($data['quote']->inhouse_date != NULL){
      $data['page'] = 'edit_order_inhouse';
  }else{
...
if($this->input->post("order_id")){
  foreach($order_ids as $key => $orderid){
    if($notes[$key] != ""){
        ...
        $this->db->insert("pnp_order_installer_activities",$data);
    }
  }
  $order= $this->ordermodel->getdata($orderid)->row();
  $notification = array(
    "content" => $this->session->userdata("username").' has added the daily activities',
    "link" => 'dashboard/activity_report/',
    "from_id" => $this->session->userdata("clms_userid"),
    "to_id" => $order->added_by,
    "added_date" => date("Y-m-d")
  );
  $this->db->insert("company_notification",$notification);
   ...
   $this->session->set_flashdata('success_message', 'Activity has been added successfully');

  redirect('dashboard/installer');
}

//need to run
function installer_data(){
  $installers = $this->db->get("pnp_order_installer")->result();
  foreach($installers as $row){
    $data = [
      "order_id" => $row->order_id,
      "installer_id" => $row->installer,
      "order_installer_id" => $row->id,
    ];
    $this->db->insert("pnp_order_assignees",$data);
  }
}

function installer_user_data(){
  $installers = $this->db->get("pnp_installer")->result();
  foreach($installers as $row){
      $user = [
    ...
    $this->db->where("threatre_id",$row->threatre_id);
    $this->db->set("user_id",$userid);
    $this->db->update("pnp_installer");
  }
}
