...
class ordermodel extends CI_Model{
	...
	function count_todayOrder(){
		...
		if($this->session->userdata("clms_company") && $this->session->userdata("clms_company") != "")
			$sql.=" and o.company_id=".$this->session->userdata("clms_company");
	...
	function count_weekOrder(){
		...
		if($this->session->userdata("clms_company") && $this->session->userdata("clms_company") != "")	$sql.=" and o.company_id=".$this->session->userdata("clms_company");
		if($alldata == 0){
			$userid = $this->session->userdata("clms_userid");
			$sql.= " and (o.added_by = $userid Or c.sales_rep = $userid)";
	...
	function total_earning($referral_id)
	{
		...
		if($this->session->userdata("clms_company") && $this->session->userdata("clms_company") != "")	
			$this->db->where("o.company_id",$this->session->userdata("clms_company"));	
		if($this->session->userdata("usergroup")  == 9)
			$this->db->where("c.referral_id",$this->session->userdata("clms_userid"));
	...
	function listall($ostatus='',$invoice='',$archive='0',$order_date='',$referral='',$from_date='',$to_date='',$period = '',$month ='',$type='',$student=null){	
		...
		$this->db->select("o.*,s.referral referral_id")->from("order o");	
		$this->db->join("company_students u","u.id = o.customer_id");
		$this->db->join("company_student_details s","u.id = s.company_student_id");
		
		if($all_data == 0){
			$this->db->where("(o.added_by = $userid OR s.referral = $company_user_id OR o.customer_id=$company_user_id )");
		}
		...
		if($student)
		 	$this->db->where("o.customer_id",$student);
	...
	function countOrderwithStatus($status,$type=null){
		...
		if($alldata == 0){
			// $this->db->where("o.added_by",$this->session->userdata("clms_userid"));
			$userid = $this->session->userdata("clms_userid");
	...
	function sendSms($orderid,$email_id){
		...
		if($this->mylibrary->getSiteEmail(73) == 1 && $row->sms == 1){
			...
			$this->db->where("company_id",$this->session->userdata("clms_userid"));
