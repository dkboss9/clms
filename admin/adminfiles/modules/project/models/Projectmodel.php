...
class Projectmodel extends CI_Model{
	...
	function listall($added_date="",$type="",$status="",$deadline="",$start_date="",$end_date="",$handle="",$filter="", $is_assigned="",$commence_date="",$student_id=null,$is_completed=null){			
		...
		$this->db->select("*,p.project_id,p.status status,p.added_date,s.first_name fname, s.last_name lname,s.id student_id")->from("projects p");
		...
		if($all_data == 0){
			$cond = "(e.student_id = $company_user_id OR e.added_by = $userid 
			OR p.sales_rep = $company_user_id OR pe.employee_id = $company_user_id OR psup.supplier_id=$company_user_id)";
				$this->db->where($cond);
		}
	...
	function getChatMessages($userid,$student_id){
		$this->db->select("*,m.added_by")->from("pnp_company_student_note_new m");
		$this->db->join("pnp_users u","u.userid=m.added_by",'left');
		$this->db->where("m.company_student_id",$student_id);
	...
	function getCollectedPoints($student_id){
		...
		$this->db->where("company_id",$this->session->userdata("clms_company"));
		return $this->db->get("pnp_enroll")->row();
	}
	...
	function getTotalInvoice($student_id){
		...
		$this->db->where("company_id",$this->session->userdata("clms_company"));
		return $this->db->get("pnp_order")->row();
	}
	...
	function get_LeadStatus(){
		...
		if($this->session->userdata("clms_company") && $this->session->userdata("clms_company") != "")	{
			$this->db->where("ls.company_id",$this->session->userdata("clms_company"));
	...
	function count_dashboardProject($month,$type_id){
		...
		if($this->session->userdata["usergroup"] != 1 && $this->session->userdata["usergroup"] != 7){
			$sql.=" and (p.sales_rep='".$this->session->userdata("clms_userid")."') OR (p.added_by='".$this->session->userdata("clms_userid")."')";
		}
		if($this->session->userdata("clms_company") && $this->session->userdata("clms_company") != "")	
			$sql.=" and p.company_id =".$this->session->userdata("clms_company");
	...
	function sendchecklistemail($chklistid,$enrollid,$content=''){
		...
		$this->email->from($sitemail, $fromname);
		$this->email->to($student->email);
		$row = $this->mylibrary->getCompanyEmailTemplate(61,$project->company_id);
		$subject = str_replace('[SITE_NAME]',$fromname,$row->email_subject);
		$subject = str_replace('[NAME]',@$company->first_name.' '.@$company->last_name,$subject);
		$subject = str_replace('[ENROL_NUMBER]',$project->order_no,$subject);
	...
	function getmailconent($id,$student){
		...
		$this->email->reply_to($data['company']->email, $fromname);
		$this->email->to( $student->email);
