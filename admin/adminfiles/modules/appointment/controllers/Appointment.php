...
function add() {
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"ADD")) {
        ...
        $data['values']['booking_date']              = $this->input->post('booking_date') != "" ? date("Y-m-d",strtotime($this->input->post('booking_date'))):null;
        $data['values']['booking_time']              =  $this->input->post('booking_date') != "" ? $this->input->post('booking_time') : null;
        ...
        $data['values']['reminder_date']              = $this->input->post('reminder_date') !="" ?  strtotime($this->input->post('reminder_date')) : null;
        ...
        if($this->input->post('status_update') !=""){
            ...
            if($this->input->post("send_email") || $type = 'Counceling'){
                ...
                $message = str_replace('[SITE_URL]',$site_url,$message);
                $message = str_replace('[LEAD_NAME]',$this->input->post('name'),$message);
                $message = str_replace('[LEAD_DETAILS]',$this->input->post('note'),$message);
                $message = str_replace('[YEAR]',date('Y'),$message);
            ...
            redirect('project/appointments/'.$this->input->post("student_id"));
                ...
                redirect('dashboard/counselling');
                    ...
                    if($this->input->post("list_type") == "calendar"){
                        redirect('appointment/calendar');
                    }else{
...
function edit() {
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"EDIT")) {
        if ($this->input->post('txt_submit')) {
            ...
            $data['values']['purpose']              = $this->input->post('purpose');
            $data['values']['from_nepal']              = $this->input->post("from_nepal");
            $data['values']['booking_date']              = $this->input->post('booking_date') != '' ? date("Y-m-d",strtotime($this->input->post('booking_date'))) : '';
            $data['values']['booking_time']              =  $this->input->post('booking_time');
            if($this->input->post("from_nepal") == 1)
                $data['values']['country']              = $this->input->post('country');
            else
                ...
                $data['values']['reminder_date']              = $this->input->post('reminder_date') != '' ? strtotime($this->input->post('reminder_date')) : '';
                $data['values']['reminder_time']              = $this->input->post("reminder_time");
            }
            $data['values']['consultant']              =  $this->input->post('counseller');
            $data['values']['appointment_status']              =  $this->input->post('appointment_status') ? $this->input->post('appointment_status') : 'Pending';
            $data['values']['added_by']                 = $userdata;
            $data['values']['added_date']           = time();
            $data['values']['modified_by']              = $userdata;
               // $data['values']['status']                   = 1;
...
function appointment_detail_calendar(){
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"EDIT")) {
        $this->load->model("lms/lmsmodel");
        ...
        $data['lead_types'] = $this->salerepmodel->get_leadType();
        $data['counselers'] = $this->projectmodel->get_empoyee();
        $query = $this->appointmentmodel->getdata($appointmentid);
        $data['result'] 	= $query->row();
        $data['booking_time_options'] = $this->lmsmodel->get_appointmenttimes($data['result'],$appointmentid);
        echo $this->load->view("edit_appointment_calendar",$data,true);
    }else{
        redirect("login");
    }
}
...
function add_consultant(){
    ...
    if($this->input->post("txt_submit")){
       ...
       $data['result'] = $query->row();
      echo $this->load->view('add_consultant', $data,true);
   }
...
function delete_appointment(){
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"DELETE")) {
        ...
        $this->usermodel->insertUserlog($logs); 
        $this->db->where("lead_id",$leadid)->delete("pnp_leads");
    }else{
        redirect('login');
    }
}

function delete_all_appointments(){
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"DELETE")) {
