...
class Course_fee extends MX_Controller {
    ...
    function add() {
        if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"ADD")) {
            ...
            $data['values']['bonus']  = $this->input->post('bonus');
            $data['values']['commission_percent']  = $this->input->post('commision');
            ...
            $txt_enrolled_per_intaked = $this->input->post("txt_enrolled_per_intaked");
            $student_from = $this->input->post("student_from");
            $student_to = $this->input->post("student_to");
            $amount = $this->input->post("amount");

            if($this->input->post('bonus') == 'Yes'){
                foreach($txt_enrolled_per_intaked as $key => $intaked){
                    if($intaked != ""){
                        $bonus = [
                            "enroll_per_intake" => $intaked,
                            "student_from" =>  $student_from[$key],
                            "student_to" => $student_to[$key],
                            "amount" => $amount[$key],
                            "course_fee_id" => $course_id
                        ];
                        $this->db->insert("course_fee_bonus",$bonus);
                    }
                }
            }
...
function edit() {
    if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"EDIT")) {
        ...
        $this->db->where("course_fee_id",$course_id)->delete("course_fee_bonus");
     ...
     $data['course_fee_bonus'] = $this->course_feemodel->course_fee_bonus($id);
