  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Tweet List</h2>
    </header>
    ...
    <table class="table table-bordered table-striped mb-none" id="datatable-default">
      ...
      <tbody>
        <?php 
        foreach ($social_media->result() as $row) {
         ...
         <tr class="gradeX">
           ...
           <td><?php echo date("d/m/Y",$row->shedule_date);?></td>
           <td><?php
          ...
          <td class="actions">
            <?php if($row->tweet_time != 3){?>
            <a href="<?php echo base_url("social_media/tweet/".$row->id);?>"><i class="fa fa-twitter"></i></a>
            <?php } ?>
            <?php echo anchor('social_media/edit/'.$row->id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('social_media/delete/'.$row->id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>social_media/add");})
    $("#checkall").click(function () { 
      $(".checkone").prop('checked', $(this).prop('checked'));
    });
    $(".link_delete").click(function(){
      if(!confirm('Are you sure to delete this social_media?'))
        return false;
    });
  });
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>social_media/cascadeAction",
          type:"post",
          data:jsonData,
