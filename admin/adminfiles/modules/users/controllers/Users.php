...
class Users extends MX_Controller{
	...
	function addgroup_popup(){
		if ($this->session->userdata("clms_userid")){
			...
			echo	$group_id = $this->db->insert_id();
			$this->usermodel->setGroup_allData($this->session->userdata("clms_company"),$group_id,$this->input->post('all_data'));
		}
	}
	function addgroup() {
		if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"ADD")) {
		...
		if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->module_code,"EDIT")) {
			if ($this->input->post('action') && $this->input->post('action') == 'update') {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$this->usermodel->setGroup_allData($this->session->userdata("clms_company"),$_POST['group_id'],$this->input->post('all_data'));  
				...
				if ($query->num_rows() > 0) {
					...
					$data['all_data'] = $this->usermodel->getGroup_allData($this->session->userdata("clms_company"),$groupid)->num_rows();
					$data['groupid'] = $row->groupid;
					$data['parent_id'] = $row->parent_id;
					$data['parents'] = $this->db->where("groupid !=",1)->where("parent_id",null)->get("pnp_group")->result();
	...
	/************* groups ******************/
	function listgroup(){
		if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->group_module_code,"VIEW")) {
...
//---------------------------------edit--------------------------------------
		function edit_group_perm() {
			if ($this->session->userdata("clms_userid") != "" && $this->usermodel->checkperm($this->group_module_code,"EDIT")) {
				if ($this->input->post('txt_permission')) {						
					$chk_permission = $_POST['chk_permission'];
					$login_id = $this->session->userdata("clms_userid"); 
					$group_id = $this->uri->segment(3);
					$company_id = $this->session->userdata("clms_company") == 0 ? NULL : $this->session->userdata("clms_company");
