    ...
    <div class="panel-body">
      <form class="form-horizontal form-bordered" id="form_discussion" action="<?php echo current_url();?>" method="post" enctype='multipart/form-data'>
       ...
       <div class="form-group">
         ...
         <div class="col-md-9">
          <select name="status" class="form-control" <?php if($this->session->userdata("clms_front_user_group") == 14) echo "disabled";?>>
          ...
          <?php if($this->session->userdata("clms_front_user_group") == 14) { ?>
            <input type="hidden" name="status" value="<?php echo $enroll_checklist->checklist_status??"" != "" ? $enroll_checklist->checklist_status : 'Pending / Not verified'; ?>">
        <?php } ?>
        </div>
      </div>
