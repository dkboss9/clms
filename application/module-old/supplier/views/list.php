      ...
      foreach ($supplier->result() as $row) {
       ...
       <tr class="gradeX">
         ...
         <td class="actions">
           <?php
           echo $publish.'&nbsp;';
           echo anchor('supplier/edit/'.$row->id,'<span class="glyphicon glyphicon-edit"></span>');
          ...
          $number = $this->employeemodel->count_requestSent($row->id,"Supplier");
          if(!$row->clms_front_userid){
          ?>
          <a href="<?php echo base_url("supplier/resend_activation/".$row->id)?>" class="link_email"><i class="fa fa-envelope" aria-hidden="true"></i>(<?php echo $number->num_rows();?>)</a>
        <?php } ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>supplier/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>supplier/cascadeAction",
          type:"post",
          data:jsonData,
