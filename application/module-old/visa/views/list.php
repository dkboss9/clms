  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">visa List</h2>
    </header>
    ...
    <table class="table table-bordered table-striped mb-none" id="datatable-default">
      ...
      <tbody>
        ...
        foreach ($access->result() as $acc) {
         ...
         $point = $this->visamodel->visaPoints($acc->type_id,$this->session->userdata("company_id"))->row();
        ?>
         <tr class="gradeX">
           ...
           <td class="actions">
            ...
            <?php 
             echo anchor('visa/edit/'.$acc->type_id,'<span class="glyphicon glyphicon-edit"></span>');
             if($acc->company_id > 0 OR $this->session->userdata("clms_front_user_group") == '1')
              echo '&nbsp;'.$publish.'&nbsp;'.anchor('visa/delete/'.$acc->type_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));
            ?>
          </td>
        </tr>
        <?php
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>visa/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>visa/cascadeAction",
          type:"post",
          data:jsonData,
