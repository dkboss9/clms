  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Employee Type List</h2>
    </header>
    ...
    <table class="table table-bordered table-striped mb-none" id="datatable-default">
      ...
      <tbody>
        ...
        foreach ($status->result() as $row) {
          ...
          ?>
          <tr class="gradeX">
           <td><input type="checkbox" class="checkone" value="<?php echo $row->type_id; ?>" /></td>
           ...
           <td class="actions">
            ...
            if($this->session->userdata("clms_front_user_group") == 1 || $this->session->userdata("clms_front_companyid") == $row->company_id)
              echo anchor('employee_type/edit/'.$row->type_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('employee_type/delete/'.$row->type_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));?>
          </td>
        </tr>
        <?php
      } ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>employee_type/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>employee_type/cascadeAction",
          type:"post",
          data:jsonData,
