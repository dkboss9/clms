             ...
             <div class="col-md-3">
               <select name="customer" data-plugin-selectTwo id="customer" class="form-control">
                 ...
                 foreach ($customers->result() as $row) {
                   ?>
                   <option value="<?php echo $row->id;?>" <?php if(@$quote->customer_id == $row->id || $row->id == $this->input->get("customer")) echo 'selected="selected"';?>><?php echo $row->first_name;?> <?php echo $row->last_name;?></option>
                   <?php
                 }
                 ?>
               </select>
...
<div class="form-group">
  ...
  <label class="col-md-2 control-label div_payment_term" <?php if(@$quote->finance_option != '1') echo 'style="display:none;"'; ?> for="price">Payment Terms</label>
