      ...
      <tbody>
        ...
        foreach ($access->result() as $acc) {
         ...
         <tr class="gradeX">
           ...
           <td><?php echo $acc->country_name;?></td>
           <td><?php echo $acc->contact_name;?></td>
           <td><?php echo $acc->contact_email;?></td>
           <td><?php echo $acc->contact_number;?></td>
           <td class="actions">

            <?php echo anchor('college/edit/'.$acc->type_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('college/delete/'.$acc->type_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));?>
            <a href="<?php echo base_url("college/send_mail/".$acc->type_id);?>" class="simple-ajax-popup123 btn-default"><span class="glyphicon glyphicon-envelope"></span></a>
          </td>
        </tr>
        <?php
      } ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>college/add");})
    ...
    $(document).on("click","#chk_sms",function(){
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>college/cascadeAction",
          type:"post",
          data:jsonData,
