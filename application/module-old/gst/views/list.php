  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Tax Type List</h2>
    </header>
      ...
      <tbody>
        ...
        foreach ($access->result() as $acc) {
         ...
         <tr class="gradeX">
           ...
           <td><?php echo $acc->gst;?></td>
           
           <td class="actions">

            <?php 
            if($acc->company_id > 0 OR $this->session->userdata("clms_front_user_group") == '1')
              echo anchor('gst/edit/'.$acc->type_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('gst/delete/'.$acc->type_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));
            ?>
          </td>
        </tr>
        <?php
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>gst/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>gst/cascadeAction",
          type:"post",
          data:jsonData,
