...
<div class="row">
			<div class="col-sm-12">
				<div class="mb-md">
					<h2>
						<a href="<?php echo base_url("custom_email_template/add")?>" data-toggle="" title="Add New User" type="button" class="btn btn-primary" id="btn-add" data-target="#ammendment" style="float:right;"> Add <i class="fa fa-plus"></i> </a> 
					</h2>
				</div>
			</div>
		</div>
		...
		<table class="table table-bordered table-striped mb-none" id="datatable-default">
			...
			<tbody>
				...
				foreach($news??[] as $key=>$value){
					?>
					<tr class="gradeX">
						<td><?php echo $news[$key]['newsid'];?></td>
						<td><?php echo $news[$key]['title'];?></td>
						<td><?php echo $news[$key]['description'];?></td>
						<td><?php if($news[$key]['sms'] == 1) echo 'Yes'; else echo 'No';?></td>
						<td class="actions">

							<?php echo anchor('custom_email_template/edit/'.$news[$key]['newsid'],'<span class="glyphicon glyphicon-edit"></span>');?>
						</td>
					</tr>
					<?php
				} ?>
