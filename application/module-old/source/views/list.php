  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">source List</h2>
    </header>
         ...
         <tr class="gradeX">
           ...
           <td class="actions">
            ...
            if($this->session->userdata("clms_front_user_group") == 1 || $this->session->userdata("clms_front_companyid") == $acc->company_id)
              echo anchor('source/edit/'.$acc->type_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('source/delete/'.$acc->type_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));?>
          </td>
        </tr>
        <?php
      } ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>source/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>source/cascadeAction",
          type:"post",
          data:jsonData,
