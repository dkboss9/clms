  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">course fee List</h2>
    </header>
    ...
    <table class="table table-bordered table-striped mb-none" id="datatable-default">
      ...
      <tbody>
        ...
        foreach ($access->result() as $acc) {
         ...
         ?>
         <tr class="gradeX">
           <td><input type="checkbox" class="checkone" value="<?php echo $acc->fee_id; ?>" /></td>
           <td><?php echo $acc->college;?></td>
           <td><?php echo $acc->degree;?></td>
           <td><?php echo $acc->course;?></td>
           <td><?php echo $acc->price;?> (<?php echo $acc->currency;?>)</td>
           <td><?php echo $acc->period;?></td>
           <td class="actions">

            <?php echo anchor('course_fee/edit/'.$acc->fee_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('course_fee/delete/'.$acc->fee_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));?>
          </td>
        </tr>
        <?php
      } ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>course_fee/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>course_fee/cascadeAction",
          type:"post",
          data:jsonData,
