...
//	echo $this->db->last_query();
	...
	function listuser_new($limit = null){
		...
		if($this->session->userdata("clms_front_companyid") && $this->session->userdata("clms_front_companyid") != "")	
			$this->db->where("ugc.company_id",$this->session->userdata("clms_front_companyid"));
	...
	function checkperm_view($module_code, $user_action_code){
	...
	function getGroup_allData($group_id,$company_id){
		$this->db->where("company_id",$company_id);
		$this->db->where("group_id",$group_id);
...
//	print_r($permissions);
	...
	function checkperm($module_code, $user_action_code){
		...
		// echo $this->db->last_query(); die();
		if($query->num_rows() >0 || $this->session->userdata("clms_front_user_group") == 1){
			return true;
		} else {
			$this->session->set_flashdata("error", "Unauthorized Access!");
			redirect('dashboard','location');
