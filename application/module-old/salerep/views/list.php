...
<section class="panel">
  <header class="panel-heading">
    ...
    <h2 class="panel-title">Sales Rep List</h2>
  </header>
      ...
      foreach ($salerep->result() as $row) {
       ...
       <tr class="gradeX">
         ...
         <td class="actions">
           <?php
           echo $publish.'&nbsp;';
           echo anchor('salerep/edit/'.$row->id,'<span class="glyphicon glyphicon-edit"></span>');
           if($row->user_group!='1' && $row->user_group!='2'){
            //echo ' <a href="'. base_url(""). 'users/perm_modify/'. $row->id .'"  class=""><span class="glyphicon glyphicon-asterisk">&nbsp;</span></a>';
          }
          if($row->user_group!='1'){
            echo  ' <a  class="icons deleteIcon" id="'.$row->id.'"><span class="glyphicon glyphicon-eye">&nbsp;</span></a>';
          }
        
          $number = $this->employeemodel->count_requestSent($row->id,"Salerep");
          if(!$row->clms_front_userid){
          ?>
          <a href="<?php echo base_url("salerep/resend_activation/".$row->id)?>" class="link_email"><i class="fa fa-envelope" aria-hidden="true"></i>(<?php echo $number->num_rows();?>)</a>
        <?php } ?>
        </td>
      </tr>
      <?php
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>salerep/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>salerep/cascadeAction",
          type:"post",
          data:jsonData,
