<?php
class Event_status extends MX_Controller {

    function __construct() {
        parent::__construct();
        $this->container = 'main';
        $this->load->model('event_statusmodel');
        $this->load->model('users/usermodel');
        $this->module_code = 'EVENT-STATUS';
    }

    function index() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
            redirect('event_status/listall', 'location');
        } else {
    ...
    function listall() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
            $data['status'] 	= $this->event_statusmodel->listall();
            $data['page'] 			= 'list';
            $this->load->vars($data);
    ...
    function add() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"ADD")) {
            ...
            $this->event_statusmodel->add($data['values']);
            $logs = array(
                "content" => serialize($data['values']),
                "action" => "Add",
                "module" => "Manage Event Status",
...
function edit() {
    if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"EDIT")) {
        if ($this->input->post('submit')) {
            ...
            $data['values']['modified_date']    = time();
            $data['values']['modified_by']      = $userdata;
            $this->event_statusmodel->update($id, $data['values']);
            $logs = array(
                "content" => serialize($data['values']),
                "action" => "Edit",
                "module" => "Manage Event Status",
            ...
            $query = $this->event_statusmodel->getdata($id);
            if ($query->num_rows() > 0) {
...
function delete() {
    if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"DELETE")) {
        ...
        $content = $this->usermodel->getDeletedData('event_status',$cond);
        $logs = array(
        ...
        $this->usermodel->insertUserlog($logs);
        $this->event_statusmodel->delete($delid);
...
function cascadeAction() {
    ...
    $query = $this->event_statusmodel->cascadeAction($ids, $action);
