  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Project Status List</h2>
    </header>
    ...
    <table class="table table-bordered table-striped mb-none" id="datatable-default">
      ...
      <tbody>
        ...
        foreach ($status->result() as $row) {
          ...
          ?>
          <tr class="gradeX">
           <td><input type="checkbox" class="checkone" value="<?php echo $row->status_id; ?>" /></td>
           ...
           <td class="actions">
            ...
            if($this->session->userdata("clms_front_user_group") == 1 || $this->session->userdata("clms_front_companyid") == $row->company_id){
              echo anchor('project_status/edit/'.$row->status_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;';
              if($row->status_id != 5)
               echo anchor('project_status/delete/'.$row->status_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));
           }
           ?>
         </td>
       </tr>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>project_status/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>project_status/cascadeAction",
          type:"post",
          data:jsonData,
