...
class Welcomemodel extends CI_Model{
	...
	function get_front_companylist($userid){
		$this->db->select("u.*,ugc.company_id,c.company_name,g.group_name,ugc.id user_group_company_id")
		->from('group g')
        ->join("user_groups ug","ug.group_id = g.groupid")
        ->join("pnp_user_group_company ugc","ugc.user_group_id = ug.id")
        ->join("users u","u.userid = ug.user_id")
		->join("users c","c.userid = ugc.company_id");
		$this->db->where("u.userid",$userid);
		return $this->db->get()->result();
	}

	function get_user_group($user_group_company_id){
		$this->db->select("ug.*,ugc.company_id,g.group_name")
		->from('group g')
        ->join("user_groups ug","ug.group_id = g.groupid")
        ->join("pnp_user_group_company ugc","ugc.user_group_id = ug.id");
		$this->db->where("ugc.id",$user_group_company_id);
		return $this->db->get()->row();
	}

	function getCompanies($userid){
		$this->db->select("u.*")->from("users u");
		$this->db->join("company_students cs","cs.company_id=u.userid");
	...
	function getProjects(){
		...
		if($this->session->userdata("clms_front_company") && $this->session->userdata("clms_front_company") != "")	
			$this->db->where("t.company_id",$this->session->userdata("clms_front_company"));
	...
	function get_chatNote(){
		...
		if($this->session->userdata("clms_front_company"))
			$this->db->where("c.company_id",$this->session->userdata("clms_front_company"));
	...
	function get_notifications(){
		...
		if($this->session->userdata("clms_front_company") && $this->session->userdata("clms_front_company") != "")	
			$this->db->where("p.company_id",$this->session->userdata("clms_front_company"));
	...
	function get_todayLeads($date = ''){
		...
		if($this->session->userdata("clms_front_company") && $this->session->userdata("clms_front_company") != "")	
			$this->db->where("l.company_id",$this->session->userdata("clms_front_company"));
	...
	function get_yesterdayLeads(){

		$this->db->select("*");
		$this->db->from("leads l");
		$this->db->join("users u","u.userid=l.user_id","left");

		//$this->db->where("l.reminder_date >= UNIX_TIMESTAMP(DATE_SUB(CURDATE(), INTERVAL 1 DAY)) AND l.reminder_date < UNIX_TIMESTAMP(CURDATE())");
	...
	//	}
		...
		if($this->session->userdata("clms_front_company") && $this->session->userdata("clms_front_company") != "")	
			$this->db->where("e.company_id",$this->session->userdata("clms_front_company"));
		if($reminded != "")
	...
	function getalerts(){
		...
		if($this->session->userdata("clms_front_company") && $this->session->userdata("clms_front_company") != "")	
			$sql.=" and company_id =".$this->session->userdata("clms_front_company");
	...
	function count_todayLead(){
		...
		if($this->session->userdata("clms_front_company") && $this->session->userdata("clms_front_company") != "")	
			$sql.=" and l.company_id =".$this->session->userdata("clms_front_company");
	...
	function count_Projects($id){
		...
		if($this->session->userdata("clms_front_company") && $this->session->userdata("clms_front_company") != "")	
			$this->db->where("company_id",$this->session->userdata("clms_front_company"));
	...
	function countInvoice($status = '',$student_id=null,$company_id=null){
		$this->db->select("*,i.status status,u.company_name")->from("order i");	
		$this->db->join("users u","u.userid=i.company_id");
		$this->db->join("company_students s","s.id=i.customer_id");
	...
	function getEnroll($time,$student_id=null,$company_id){
