	...
	function sendemail(){
		// $this->load->library('email');
		   ...
		   $config['mailtype'] = "html";
			   ...
			   $config['smtp_user'] = "developer.khrouch@gmail.com"; 
			   $config['smtp_pass'] = "Khrouch@123#";
	   ...
	   $this->email->from('developer.khrouch@gmail.com', 'Khrouch');
	   $this->email->to('bikash.suwal01@gmail.com');
	   
	   
	   $this->email->subject('Email Subject here');
	   $this->email->message('thos email class.');
	   if (!$this->email->send())
		...
		if($this->session->userdata("clms_front_userid")!="" && $this->session->userdata('username')!=""){
			...
			if($this->input->post('submit')){ 
				...
				if($this->form_validation->run()==FALSE){
					...
					$query = $this->loginmodel->checkuser_new($username,$password);
				//echo $this->db->last_query();die();
					...
					$userdata = array(
						'clms_front_userid'=>$row->userid,
						'clms_front_email'=>$username,
						'clms_front_chatname'=>$row->user_name,
						'clms_front_username'=>$row->first_name.' '.$row->last_name,'
						front_logid'=>$lastid);
					$query->free_result();
					//print_r($userdata); die();
					$this->session->set_userdata($userdata);
					redirect('project/dashboard','location');
				}else{
