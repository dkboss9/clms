...
class order_status extends MX_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->container = 'main';
        $this->load->model('order_statusmodel');
        $this->load->model('users/usermodel');
        $this->module_code = 'order_status';
    }

    function index() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
            redirect('order_status/listall', 'location');
        } else {
    ...
    function listall() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
            $data['status'] 	= $this->order_statusmodel->all_order_status();
            $data['page'] 			= 'list';
            $this->load->vars($data);
    ...
    function add() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"ADD")) {
            if ($this->input->post('submit')) {
                ...
                $this->order_statusmodel->add($data['values']);

                $logs = array(
                    "content" => serialize($data['values']),
                    "action" => "Add",
                    "module" => "Manage order_status",
                    "added_by" => $this->session->userdata("clms_front_userid"),
                    "added_date" => time()
                    );
                $this->usermodel->insertUserlog($logs);

                $this->session->set_flashdata('success_message', 'Order Status added successfully');
                redirect('order_status/listall');
            }else{
                $data['page'] = 'add';
    ...
    function edit() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"EDIT")) {
            if ($this->input->post('submit')) {
                ...
                $data['values']['modified_date']    = time();
                $data['values']['modified_by']      = $userdata;
                $this->order_statusmodel->update($id, $data['values']);
                $logs = array(
                    "content" => serialize($data['values']),
                    "action" => "Edit",
                    "module" => "Manage order_status",
                ...
                $this->session->set_flashdata('success_message', 'Order Status edited Successfully');
                redirect('order_status/listall');
            } else {
                $id = $this->uri->segment(3);
                $query = $this->order_statusmodel->getdata($id);
                if ($query->num_rows() > 0) {
    ...
    function delete() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"DELETE")) {
            ...
            $content = $this->usermodel->getDeletedData('order_status',$cond);
            $logs = array(
            ...
            $this->usermodel->insertUserlog($logs);
            $this->order_statusmodel->delete($delid);
    ...
    function cascadeAction() {
        ...
        $query = $this->order_statusmodel->cascadeAction($ids, $action);
