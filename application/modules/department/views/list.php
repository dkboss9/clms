    ...
    <tbody>
      ...
      foreach ($departments->result() as $row) {
       ...
       <td><input type="checkbox" class="checkone" value="<?php echo $row->id; ?>" /></td>
         ...
         <td class="actions">
              <?php echo anchor('department/edit/'.$row->id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('department/delete/'.$row->id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));?>
              <!-- <a href="<?php echo base_url("department/edit_department_perm/". $row->id); ?> " title="Set Group Permission"><span class="glyphicon glyphicon-th-large"></span></a> -->
        </td>
      </tr>
      <?php
    } ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>department/add");})
