...
function edit() {
  if ($this->session->userdata("clms_front_userid") != "" ) {
    if ($this->input->post('student_id')) {
     ...
     $value['details']['added_by']       = $this->session->userdata("clms_front_userid");
     $value['details']['status']         = 1;
     $this->db->where("userid",$student);
     $this->db->update("users",$value['details']);
     ...
     $this->db->update("student_details",$insert_array);
  ...
  //  print_r( $type[0]); die();
     foreach($_FILES as $key => $value) {
      ...
      //print_r($errors);
          ...
          foreach($uploads as $key => $value){ 
            $image = $value['file_name'];
            $student_documents = array("student_id"=>$student,"doc_type"=>$type1,"doc_name"=>$image,'doc_desc'=>$desc1);
