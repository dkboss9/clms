  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Visa Subclass List</h2>
    </header>
         ...
         <tr class="gradeX">
           ...
           <td class="actions">
            ...
            if($acc->company_id > 0 OR $this->session->userdata("clms_front_user_group") == '1')
              echo anchor('visa_class/edit/'.$acc->type_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('visa_class/delete/'.$acc->type_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));
            ?>
          </td>
        </tr>
        <?php
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>visa_class/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>visa_class/cascadeAction",
          type:"post",
          data:jsonData,
