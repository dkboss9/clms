...
function select_company(){
    if ($this->session->userdata("clms_front_userid") != "") {
        $companyid = $this->input->post("root_company");
        $this->session->set_userdata("clms_front_companyid",$companyid);
        redirect("appointment/add");
    }else{
        redirect("login");
    }
}
...
function add() {
    if ($this->session->userdata("clms_front_userid") != "") {
        ...
        $data['values']['modified_by'] 				= $userdata;
        $data['values']['status'] 					= 1;
        $data['values']['appointment_status'] 					= 'Pending';
        $this->appointmentmodel->add($data['values']);
        $leadid = $this->db->insert_id();
        ...
        if($this->input->post('status_update') !=""){
            ...
            /************* send activation email to customer ************/
              ...
              //  $this->email->cc('bikash.suwal01@gmail.com');
                $row = $this->mylibrary->getCompanyEmailTemplate(88,$this->session->userdata("clms_front_companyid"));
                ...
                $message = str_replace('[LEAD_NAME]',$this->input->post('name'). ' '.$this->input->post("lname"),$message);
           ...
           $data['student'] = $this->studentmodel->getdata_new($this->session->userdata("clms_front_userid"))->row();
...
function edit() {
    if ($this->session->userdata("clms_front_userid") != "") {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($query->num_rows() > 0) {
                $data['result'] 	= $query->row();
                if($this->session->userdata("clms_front_companyid") != $data['result']->company_id){
                    $this->session->set_userdata("clms_front_companyid",$data['result']->company_id);
                    redirect("appointment/edit/".$id);
                }
                if($this->input->get("tab") == 'counselling')
