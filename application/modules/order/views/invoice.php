	<table border="0" width="100%"  style="border-collapse: collapse;">
		<tr>
			<td colspan="2" >
				<h2>Invoice : #<?php echo $result->order_number; ?></h2>
		...
		<tr>
			...
			<td colspan="3" style="text-align:right;font-size:14px;">
				...
				<br/>
				<strong>Due Date:</strong> <?php echo date("d/m/Y",strtotime($result->expiry_date));?>
			</td>
		</tr>
		<tr>
			<td colspan="5">
	...
	<!-- 	<tr>
		...
		foreach ($quote_inverters as $row ) {
			...
			<td  colspan="2" style="text-align:right;font-size:12px;">
				<p ><span style="border-bottom:1px dotted #0000000;">Regular Price: <?php echo number_format($result->qprice,2);?>(<?php echo @$account_setting->currency_code;?>)</span></p>
				<?php
				...
				?>
				<p><span style="border-bottom:1px dotted #0000000;">Sub Total: <?php echo number_format($subtotal,2);?>(<?php echo @$account_setting->currency_code;?>)</span> </p>
				<?php if($result->gst_applicable == 1){ ?>
				<p><span style="border-bottom:1px dotted #0000000;"> <?php echo $result->is_included == 1 ? '':'Including'; ?> <?php echo @$account_setting->gst_title; ?>: <?php echo number_format($result->gst,2);?> <?php echo @$account_setting->currency_code;?></span></p>
				<?php }else{
					?>
					<p><span style="border-bottom:1px dotted #0000000;">  <?php echo @$account_setting->gst_title; ?>: - </span></p>
					<?php
				} ?>
				<p><span style="border-bottom:1px dotted #0000000;">Total Price: <?php echo number_format($result->price,2);?>(<?php echo @$account_setting->currency_code;?>)</span></p>
				<p><span style="border-bottom:1px dotted #0000000;">Paid: <?php echo number_format($result->price - $result->due_amount,2);?>(<?php echo @$account_setting->currency_code;?>)</span></p>
				<p><span style="border-bottom:1px dotted #0000000;color:red;">Due:	<?php echo number_format($result->due_amount,2);?>(<?php echo @$account_setting->currency_code;?>)</span></p>
				...
				<p style="border:1px solid red;text-align:center;">With Deposit of <?php echo number_format($result->price/$result->payment_term); ?>(<?php echo @$account_setting->currency_code;?>) will go over <?php echo $result->payment_term;?> months.</p>
				<?php } ?>
				...
				<p>A minimum deposit of <?php echo $result->minimum_deposit;?>% (<?php echo number_format(($result->price * $result->minimum_deposit)/100,2); ?> <?php echo @$account_setting->currency_code;?>) is required to proceed with this invoice.
				</p>
				<?php } ?>
