<?php
class sub_agentsmodel extends CI_Model{
	function __construct(){
		parent::__construct();
		$this->table = 'sub_agents';		
	...
	function send_approval_email($sub_agent_id){
		$agent = $this->getdata($sub_agent_id)->row();
	...
	//	$fromname = $this->mylibrary->getSiteEmail(20);
		...
		$subject = str_replace('[GROUP_NAME]','Sub agent',$subject);
		...
		$message = str_replace('[GROUP_NAME]','Sub agent',$message);
		...
		$message = str_replace('[LINK]','<a href="'.SITE_URL.'invite-user/'.$company->uuid.'/'.$agent->uuid.'?type=20">Accept Invitation</a>',$message);
