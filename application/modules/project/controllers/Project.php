...
function edit_checklist($chklistid,$enrollid){
    ...
    $insert_array = array(
      ...
      if(!empty($docs)){
        foreach($docs as $doc){
          ...
          $this->db->where("student_id",$this->session->userdata("clms_front_userid"));
          $student_docs = $this->db->get("pnp_student_documents")->result();
          foreach($student_docs as $sdoc){
            $this->db->insert("pnp_enroll_checklist_files",[
              "enroll_checklist_id" => $enroll_checklist_id,
              "checklist_file" => $sdoc->doc_name
              ]);
          }
        }
      }
  ...
  redirect($_SERVER['HTTP_REFERER']);
   ...
   if ($query->num_rows() > 0) {
    ...
    $data['doc_types'] = $this->studentmodel->get_docType();
...
function profile($student_id=null,$enroll_id=null){
  $student_id = $this->session->userdata("clms_front_userid");
  $this->load->model("student/studentmodel");
  if($this->session->userdata("clms_front_userid")){
    $data['result'] =  $this->studentmodel->getdata_new($student_id)->row();
...
function cases($student_id=null){
  $student_id = $this->session->userdata("clms_front_userid");
...
function chatmsglist(){
  if($this->session->userdata("clms_front_userid")){
    $student_id = $this->session->userdata("clms_front_userid");
     $data['companylist'] = $this->projectmodel->chatmsglist();
     $data['result'] =  $this->studentmodel->getdata_new($student_id)->row();
...
function notes($student_id=null){
  if($this->session->userdata("clms_front_userid")){
    $front_student_id = $this->session->userdata("clms_front_userid");
     $data['messages'] = $this->projectmodel->getChatMessages($student_id);
     $data['result'] =  $this->studentmodel->getdata_new($front_student_id)->row();
...
function add_customerNote(){
  if($this->session->userdata("clms_front_userid")){
    ...
    $company_student = $this->studentmodel->getdata($student_id)->row();

    $this->db->insert("pnp_company_student_note_new",[
      "to_id" =>$company_student->company_id,
      "comment" => $note,
      "file_name" => $file_name,
    ...
    $company = $this->quotemodel->getCompanyDetails($company_student->company_id);
    ...
    if($customer->student_id > 0){
      ...
      $sendemail   = $this->mylibrary->getSiteEmail(22);
      $this->email->from($sendemail, $fromname);
      $this->email->reply_to($customer->email, $fromname);
    //  $company->email = 'bikash.suwal01@gmail.com';
      ...
      $subject = str_replace('[FROM_NAME]', $customer->first_name.' '.$customer->last_name, $subject);
      $subject = str_replace('[SITE_NAME]', $fromname, $subject);
      $subject = str_replace('[FULL_NAME]', $company->company_name, $subject);
      ...
      $message = str_replace('[FROM_NAME]', $customer->first_name.' '.$customer->last_name, $message);
      $message = str_replace('[SITE_NAME]', $fromname, $message);
      $message = str_replace('[CONTENT]', $note, $message);
      $message = str_replace('[FULL_NAME]', $company->company_name, $message);
      ...
      $query = $this->smsmodel->getdata($company_student->company_id);
...
function doc_type($student_id=null){
  $student_id = $this->session->userdata("clms_front_userid");
  if($this->session->userdata("clms_front_userid")){
    $data['result'] =  $this->studentmodel->getdata_new($student_id)->row();
    $data['student_id'] = $student_id;
...
function add_doctype(){
  ...
  if($this->session->userdata("clms_front_userid")){
   ...
   $typeid = $this->input->post("doc_typeid");
   if($typeid == ''){
      $this->db->insert("pnp_doc_type",[
        "type_name"=> $title,
...
function delete_doctype($id){
  $student_id = $this->session->userdata("clms_front_userid");
  if($this->session->userdata("clms_front_userid")){
    $doc_type = $this->db->where("type_id",$id)->get("pnp_doc_type")->row();
    if($doc_type->company_id != $student_id){
      $this->session->set_flashdata("error_message","You don't have permmission to delete this record.");
      redirect("project/doc_type");
    }
    $docs = $this->db->where("doc_type",$id)->get("pnp_student_documents");
    if($docs->num_rows() > 0){
      $this->session->set_flashdata("error_message","Document type with documents couldn't be deleted.");
      redirect("project/doc_type");
    }

    $this->db->where("type_id",$id)->delete("doc_type");
    $this->session->set_flashdata("success_message","Document type deleted successfully.");
    redirect("project/doc_type");
  }else{
...
function points($student_id=null){
  $student_id = $this->session->userdata("clms_front_userid");
  if($this->session->userdata("clms_front_userid")){
    $data['result'] =  $this->studentmodel->getdata_new($student_id)->row();
    $data['student_id'] = $student_id;
...
function documents($student_id=null){
  $student_id = $this->session->userdata("clms_front_userid");
  $this->load->model("student/studentmodel");
  $this->load->model("order/ordermodel");
  $this->load->model("quote/quotemodel");
      ...
      foreach($_FILES as $key => $value) {
       ...
       //print_r($errors);
           ...
           foreach($uploads as $key => $value){ 
             ...
             $student_documents = array("student_id"=>$student_id,"doc_type"=>$type1,"doc_name"=>$image,'doc_desc'=>$desc1);
...
function appointments($student_id=null){
  $this->load->model("student/studentmodel");
  $appointment_date = $this->input->get("appointment_date"); 
  $student_id = $this->session->userdata("clms_front_userid");
 
  if($this->session->userdata("clms_front_userid")){
    $data['root_companies'] = $this->projectmodel->get_rootCompanies();
