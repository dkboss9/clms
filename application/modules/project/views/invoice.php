          ...
          <table class="table table-bordered table-striped mb-none" id="datatable-default" style="width: 1700px;">
            <thead>
              <tr>
                ...
                <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Actions" style="width: 10%;">Actions
                </th>
                <th>Order Number</th>
                <th>Company Name</th>
            ...
            <tbody>
              ...
              <tr class="gradeX">
                ...
                <td class="actions">
                  ...
                  <a href="<?php echo base_url("order/details/".$row->order_id);?>?project=1" class=""><span
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    function calculate_hourly_total() {
      ...
      var total = parseFloat(total) + parseFloat(fuel_amount) + parseFloat(transport_amount) + parseFloat(
        others_amount);
    ...
    function calculate_flat_total() {
      ...
      var transport_amount = $("#transport_amount").val();
      var others_amount = $("#others_amount").val();
      var total = parseFloat(flat_amount) + parseFloat(fuel_amount) + parseFloat(transport_amount) + parseFloat(
        others_amount);
      $("#total_amount").val(total);
    }
    ...
    $('.glyphicon-download-alt').tooltip({
      'placement': 'bottom',
      'title': 'Download Order confirmation'
    });
    $('.glyphicon-fullscreen').tooltip({
      'placement': 'bottom',
      'title': 'Preview invoice'
    });
    $('.glyphicon-envelope').tooltip({
      'placement': 'bottom',
      'title': 'Send Order conirmation'
    });
    $('.fa-eye').tooltip({
      'placement': 'bottom',
      'title': 'Order Seen'
    ...
    $('#addButton').bind('click', function () {
      window.location.assign("<?php echo base_url() ?>order/add?tab=1");
    })
  ...
  function cascade(action) {
    ...
    confirmation("Confirmation", "Are you sure you want to proceed this Action? ", "Cancel", "Confirm", "", action,
      "<?php echo base_url(); ?>order/cascadeAction");
  ...
  $(document).ready(function () {
    ...
    $(document).on("click", "#add_more", function () {
      ...
      $("#div_document").append('<tr style="margin-top:10px;" id="tr_' + num + '">' +
        '<td> <input type="text" name="package_desc[]" value="" rel="' + num + '" id="package_desc_' + num +
        '" class="package_desc form-control" > </td>' +
        '<td style="padding-left:10px;"> <input type="text" name="short_desc[]" value="" rel="' + num +
        '" id="short_desc_' + num + '" class="short_desc form-control" > </td>' +
        '<td> <input type="text" name="package_qty[]" value="1" rel="' + num + '" id="package_qty_' + num +
        '" class="package_qty form-control number" style="width:50px;margin-left:20px;"></td>' +
        '<td><input type="text" name="package_price[]" value="0" rel="' + num + '" id="package_price_' + num +
        '" class="package_price form-control number" style="width:100px;margin-left:20px;"></td>' +
  ...
  '') {
    ...
    var confirmModal =
      $('<div class="modal" >' +
        ...
        '<h3><span class="modal-icon" style="width:10%;float:none;"><i class="fa fa-question-circle"></i></span><strong>' +
        heading + '</strong></h3>' +
        '</div>' +
