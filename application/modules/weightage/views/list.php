  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Lead Weightage List</h2>
    </header>
    ...
    <table class="table table-bordered table-striped mb-none" id="datatable-default">
      ...
      <tbody>
        <?php 
        foreach ($weightage->result() as $row) {
         ...
         ?>
         <tr class="gradeX">
           <td><input type="checkbox" class="checkone" value="<?php echo $row->weightage_id; ?>" /></td>
           ...
           <td class="actions">
            ...
            if($this->session->userdata("clms_front_user_group") == 1 || $this->session->userdata("clms_front_companyid") == $row->company_id)
              echo anchor('weightage/edit/'.$row->weightage_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('weightage/delete/'.$row->weightage_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));?>
          </td>
        </tr>
        <?php
      } ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>weightage/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>weightage/cascadeAction",
          type:"post",
          data:jsonData,
