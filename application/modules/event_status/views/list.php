  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Event Status List</h2>
    </header>
        ...
        foreach ($status->result() as $row) {
          ...
          <tr class="gradeX">
           ...
           <td class="actions">
            ...
            <?php 
            if($row->status_id != 5)
              echo anchor('event_status/edit/'.$row->status_id,'<span class="glyphicon glyphicon-edit"></span>').'&nbsp;'.$publish.'&nbsp;'.anchor('event_status/delete/'.$row->status_id,'<span class="glyphicon glyphicon-trash"></span>',array('class'=>"link_delete"));
            else
              echo $publish;
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addButton').bind('click',function(){window.location.assign("<?php echo base_url() ?>event_status/add");})
    ...
    function cascade(action){
      ...
      if(ids.length == 0){
        ...
        $.ajax({
          url:"<?php echo base_url(); ?>event_status/cascadeAction",
          type:"post",
          data:jsonData,
