<?php
class Lms_project_status extends MX_Controller {

    function __construct() {
        parent::__construct();
        $this->container = 'main';
        $this->load->model('lms_project_statusmodel');
    ...
    function index() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
            redirect('lms_project_status/listall', 'location');
        } else {
    ...
    function listall() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"VIEW")) {
            $data['status'] 	= $this->lms_project_statusmodel->listall();
            $data['page'] 			= 'list';
            $this->load->vars($data);
    ...
    function add() {
        if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"ADD")) {
            ...
            $this->lms_project_statusmodel->add($data['values']);
            $logs = array(
            ...
            redirect('lms_project_status/listall');
        }else{
            $data['page'] = 'add';
...
function edit() {
    if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"EDIT")) {
        if ($this->input->post('submit')) {
            ...
            $data['values']['modified_date']    = time();
            $data['values']['modified_by']      = $userdata;
            $this->lms_project_statusmodel->update($id, $data['values']);
            $logs = array(
                "content" => serialize($data['values']),
                "action" => "Edit",
                "module" => "Manage Project Status",
            ...
            $query = $this->lms_project_statusmodel->getdata($id);
            if ($query->num_rows() > 0) {
...
function delete() {
    if ($this->session->userdata("clms_front_userid") != "" && $this->usermodel->checkperm($this->module_code,"DELETE")) {
        ...
        $content = $this->usermodel->getDeletedData('lms_project_status',$cond);
        $logs = array(
        ...
        $this->usermodel->insertUserlog($logs);
        $this->lms_project_statusmodel->delete($delid);
...
function cascadeAction() {
    ...
    $query = $this->lms_project_statusmodel->cascadeAction($ids, $action);
